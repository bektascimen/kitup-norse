  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('quizzes')
        .select(
          'id, lesson_id, pass_threshold, quiz_questions(id, type, stem_key, explanation_key, position, quiz_options(id, label_key, is_correct, position))',
        )
        .eq('id', quizId!)
        .single();
      ...
      const questions = (data.quiz_questions ?? [])
        ...
        .map((q: any) => ({
          ...
          correctOptionIds: (q.quiz_options ?? [])
